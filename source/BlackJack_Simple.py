import random

class deck:
    def __init__(self):
        self.cards = []
        suits = ['spades', 'club', 'diamond', 'hearts']
        ranks = [
            {"rank": "A", "value": 11},
            {"rank": "2", "value": 2},
            {"rank": "3", "value": 3},
            {"rank": "4", "value": 4},
            {"rank": "5", "value": 5},
            {"rank": "6", "value": 6},
            {"rank": "7", "value": 7},
            {"rank": "8", "value": 8},
            {"rank": "9", "value": 9},
            {"rank": "10", "value": 10},
            {"rank": "J", "value": 10},
            {"rank": "Q", "value": 10},
            {"rank": "K", "value": 10}
        ]
        for suit in suits:
            for rank in ranks:
                self.cards.append(card(suit, rank))

    def shuffle(self):
        if (len(self.cards) > 1):
            random.shuffle(self.cards)

    def deal(self, number):
        cards_dealt = []
        for x in range(number):
            if (len(self.cards) > 0):
                card = self.cards.pop()
                cards_dealt.append(card)
        return cards_dealt

    pass


class card:
    def __init__(self, suit, rank):
        self.suit = suit
        self.rank = rank

    def __str__(self):
        return f"{self.rank['rank']} of {self.suit}"
    def reset(self):
        self.suit=0
        self.rank['rank']=''
        self.rank['value']=0
    pass


class hand:
    def __init__(self, dealer=False):
        self.cards = []
        self.value = 0
        self.dealer = dealer

    def add_card(self, card_list):
        self.cards.extend(card_list)

    def calculate_value(self):
        self.value = 0
        has_ace = False
        for card in self.cards:
            card_value = int(card.rank["value"])
            self.value += card_value
            if (card.rank["rank"] == "A"):
                has_ace = True
        if has_ace and self.value > 21:
            self.value -= 10

    def get_value(self):
        self.calculate_value()
        return self.value

    def is_blackjack(self):
        return self.get_value() == 21

    def display(self, show_all_dealer_card=False):
        print(f'''{"Dealer's" if self.dealer else "Your"} hand ''')
        for index, card in enumerate(self.cards):
            if (index == 0 and self.dealer and not show_all_dealer_card and not self.is_blackjack()):
                print('hidden')
            else:
                print(card)

        if not self.dealer:
            print("Value :", self.get_value())
        print()

    pass


class game:
    def play(self):
        game_attempt = 0
        game_to_play = 0
        while (game_to_play <= 0):
            try:
                game_to_play = int(input("How many games do you want to play ?\nYour answer : "))
            except:
                print("You must input a  positive interger number")
        while (game_attempt < game_to_play):

            game_attempt += 1
            d = deck()
            d.shuffle()
            player_hand = hand()
            dealer_hand = hand(dealer=True)

            for i in range(2):
                player_hand.add_card(d.deal(1))
                dealer_hand.add_card(d.deal(1))
            print()
            print("*" * 40)
            print(f"Game {game_attempt} of {game_to_play}")
            print("*" * 40)
            player_hand.display()
            dealer_hand.display()

            if (self.check_for_winner(player_hand, dealer_hand)):
                print('\tShow both 2 players cards : ')
                player_hand.display()
                dealer_hand.display(show_all_dealer_card=True)
                continue

            choice = ""
            while (player_hand.get_value() < 21 and choice not in ['s', 'stand']):
                choice = input('Please choose Hit or Stand : ').lower()
                print()
                while (choice not in ['h', 's', 'hit', 'stand']):
                    choice = input('Please choose Hit or Stand or H/S : ').lower()
                    print()
                if (choice in ['hit', 'h']):
                    player_hand.add_card(d.deal(1))
                    player_hand.display()

            if (self.check_for_winner(player_hand, dealer_hand)):
                print('\nShow both 2 players cards : ')
                player_hand.display()
                dealer_hand.display(show_all_dealer_card=True)
                continue

            player_hand_value = player_hand.get_value()
            dealer_hand_value = dealer_hand.get_value()

            while (dealer_hand_value < 17):
                dealer_hand.add_card(d.deal(1))
                dealer_hand_value = dealer_hand.get_value()

            dealer_hand.display(show_all_dealer_card=True)

            if (self.check_for_winner(player_hand, dealer_hand)):
                print('\nShow both 2 players cards : ')
                player_hand.display()
                dealer_hand.display(show_all_dealer_card=True)
                continue

            print('Final result : ')
            print('Your hand :', player_hand_value)
            print("Dealer 's hand", dealer_hand_value)
            print('\n')
            self.check_for_winner(player_hand, dealer_hand, game_over=True)

        print('\nEND, THANKS FOR PLAYING ')

    def check_for_winner(self, player_hand, dealer_hand, game_over=False):
        if (not game_over):
            if (player_hand.get_value() > 21):
                print("\tYou lost, dealer wins :( ")
                return True
            elif (dealer_hand.get_value() > 21):
                print("\tDealer lost, you win :) ")
                return True
            elif (dealer_hand.is_blackjack() and player_hand.is_blackjack()):
                print("\tBoth player got blackjack, tie ! ")
                return True
            elif (player_hand.is_blackjack()):
                print('\tYou got blackjack, you win, dealer lost :) ')
                return True
            elif (dealer_hand.is_blackjack()):
                print('\tDealer got blackjack, dealer wins, you lost :( ')
                return True
        else:
            if (player_hand.get_value() > dealer_hand.get_value()):
                print('\tYou win, dealer lost :) ')
            elif (player_hand.get_value() < dealer_hand.get_value()):
                print('\tDealer win, you lost :( ')
            elif (player_hand.get_value() == dealer_hand.get_value()):
                print('\tTie ! ')
            return True
        return False

    pass


#g = game()
#g.play()



